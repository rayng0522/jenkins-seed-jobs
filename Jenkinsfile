        ...
        stage('Multibranch project') {
            steps {
                ...
                script {
                    ...
                    def result = sh(script: "jobDsl( targets: ['multibranch.groovy'].join('\n'), additionalParameters: [ jobs: jobs, blueprintsFolder: 'RT-SRE/blueprints', remoteJenkinsfileGitRepoUrl: \"https://github.com/rayng0522/jenkins-seed-jobs.git\", gitCredential: 'ntwairay'])", returnStdout: true)
                    echo "Result"
                    echo result
                }
            }
